...
| Render 公网部署 | `render.yaml`(Web Service, Docker, free, /healthz, checksPass) | `tests/test_deploy_check.py`(9 tests) + `tests/service/test_healthz.py`(3 tests) + `tests/service/test_demo_mode.py`(7 tests) | Docker 容器 Demo Mode 验证通过;`make deploy-check` 本地验证脚本就绪 | 待平台配置 |
| 健康检查 | `aegiscode/service/api.py::healthz`(`GET /healthz` → `{"status":"ok","service":"aegiscode","mode":"..."}`) | `tests/service/test_healthz.py`(`test_healthz_returns_200` / `test_healthz_does_not_leak_secrets` / `test_healthz_has_mode_field`) | Docker 容器内 `curl /healthz` = 200 | ✅ 通过 |
...
| 部署验证命令 | `Makefile::deploy-check` + `scripts/deploy_check.py` | `tests/test_deploy_check.py`(9 tests: healthz/secrets/webui/main 逻辑) | 公网 URL 获得后执行 `make deploy-check DEPLOY_URL=...` | 待公网验证 |
| CI/CD 联动 | GitHub Actions CI + Render GitHub Integration (checksPass) | CI pass → Render auto-deploy | Render Service 连接后验证 | 待平台配置 |
