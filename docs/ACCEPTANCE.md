...
**测试计数演进（历史，便于对照）**:M0-M7 核心 32 task 完成时 325 → Render 公网部署（原计划要求）后 344 → WebUI 预设 Demo 迭代（75 个演示测试）后 419 → 真实 LLM Provider 追加实现（Milestone 8,T33-T40）后 **444**。下方矩阵中部分早期行内仍引用当时任务验证时的历史计数（如 325/419），保留为历史事实，不回改。
...
**追加实现**：真实 LLM Provider 可用性（PR #16/#17/#18,DeepSeek 端到端人工验证 PASS）。真实 LLM 验证为人工触发，**不替代** MockLLM 测试与 `make demo`。
